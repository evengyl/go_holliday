<header id="head" style="background:#181015 url(<?= $slides; ?>) no-repeat center;">
	<div class="container">
		<div class="row">
			<h1 class="col-12 lead">Bienvenue sur <?= $_app->site_name; ?></h1>
			<p class="col-12 tagline"><a style="background: #0008; padding: 4px;">&nbsp;&nbsp;&nbsp;Votre site de recherche gratuit pour trouver vos vacances&nbsp;&nbsp;&nbsp;</a></p>
			<p class="col-12"><a class="btn btn-success" href="/Recherche" role="button">Chercher ce dont vous rêvez</a></p><?
			if($_app->option_app['app_with_login_option'])
			{
				if(!Config::$is_connect)
					echo '<p class="col-12"><a class="btn btn-warning" href="/Inscription" role="button">Vous inscrire</a></p>';
			}?>
		</div>
	</div>
</header>

<div class="container">
	<div class="text-center">
		<br><br>
		<h2 class="thin ">Nous réalisons vos envies de vacances simple et directement en ligne</h2>
		<p class="text-muted text-center">
			Nous vous proposons une interface simple et intuitive pour chercher / trouver / réservez vos vacances en familles ou en couples.</br>
			Vous aurez accès a une interface gratuite et sans limite d'utilisation pour gérer vos vacances car :<br> nous vous mettons directement en relation avec un propriétaire 
			de bungalow, appartement, maison d'hôtes, maison de vacances.
		</p>
	</div>
</div>

__MOD2_search_type__


__MOD_reason_why__


__MOD_faq__